...
while t < 120:
    ...
    sensor_angles = angles_calcu(sensors_position_vector[:, 0], sensors_position_vector[:, 1], robot_position[0],
                                 robot_position[1])
    ...
    if theta != 0.0 and (0.7 >= exact_dist[3] > 0.01 or 0.7 >= exact_dist[4] > 0.01):
        ...
        if v == 0:
            ...
            t_turn = min(random.random()*3, abs(theta)/2*v)
        print('t_turn', t_turn)
        print('v_left', v_left)
