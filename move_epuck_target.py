...
while current_time - loop_start_time < 200:  # Main loop
    ...
    for i in range(num_robots):  # measure control parameter for each robot
        ...
        if not robots[i].obj_avoid and not (prev_cal_mag[i] and prev_cal_angle[i]):  # To calculate center of mass
            ...
            if current_time - loop_start_time < 8:
                com_x, com_y = robots[i].flock_com(vision_max_grad, vision_grad_theta, 0.0, 1)  # New COM calculated in terms of co-ordinates
            else:
                com_x, com_y = robots[i].flock_com(vision_max_grad, vision_grad_theta, -1.0, 1)  # New COM calculated in terms of co-ordinates
            curr_com_mag, curr_com_angle = angles_calcu(com_x, com_y, 0, 0)  # New COM calculated in terms vector
            prev_cal_mag[i].append(clamp(curr_com_mag, 0.05, 0.12))  # update variable of previous com magnitude
