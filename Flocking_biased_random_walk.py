...
while current_time - loop_start_time < 500:  # Main loop
    ...
    for i in range(num_robots):  # measure control parameter for each robot
        ...
        if not robots[i].obj_avoid and not (prev_cal_mag and prev_cal_angle):
            ...
            if i == 11:  # last robot is leading robot
                com_x, com_y = robots[i].flock_com(target_mag, target_angle, 0, 1/6)  # New COM calculated in terms of co-ordinates
            else:  # following robot
                com_x, com_y = robots[i].flock_com(target_mag, target_angle, 1, 0)  # New COM calculated in terms of co-ordinates
        ...
        if str_mov[i] or robots[i].obj_avoid:  # robot will rotate for flocking or for obj avoid
            ...
            v_l[i], v_r[i], rot_t[i] = robots[i].rotation_robot(rot_command, 1)  # regulation factor can be given as input
