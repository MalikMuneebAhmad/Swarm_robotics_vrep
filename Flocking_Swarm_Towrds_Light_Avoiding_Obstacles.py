...
execu_time = int(input("Enter Enter Execution time "))
#num_robots = 5
...
min_safe_dist = 0.11  # Minimum safe distance for sensor
max_safe_dist = 0.14  # Maximum safe distance for sensor
...
flock_perf = 1
desired_dist_traveled = 2.2  # Distance Traveled by flock
dist_traveled = 0.0  # Distance Traveled by flock
...
#while current_time - loop_start_time < exec_time:  # Main loop
while dist_traveled < desired_dist_traveled and vrep_t < execu_time:
    print('waiting.............................................................')
    ...
    print('Maximum Cluster Size', max_cluster_size)
    ...
    for i in range(num_robots):
        ...
        robots[i].sensor_avoidance_values(0.15, sensor_range)  # modified sensor values for object avoidance
        ...
        if avg_ir_value[i] < 0.1 and not robots[i].semi_mature_flock:  # avg intensity of 8 sensors is low in black area
            obj_theta = robots[i].object_avoidance(robots[i].det_obj_handles, max_avoid_dist)
        ...
        obj_theta = robots[i].object_avoidance(robots[i].static_object_handles, max_avoid_dist)
        rot_theta = obj_theta
        ...
        if not robots[i].obj_avoid and not (prev_cal_mag[i] and prev_cal_angle[i]):  # To calculate center of mass
            ...
            vision_max_grad = clamp(vision_max_grad, 0.1, 0.25)  # to keep values of velocity into limit
            if not robots[i].det_rob:
                vision_max_grad = 3  # No robot move fastly
            ...
            if robots[i].semi_mature_flock:
                ...
                print('for robot {} semi mature flocking is {}'.format(i, robots[i].semi_mature_flock))
            if current_time - loop_start_time < 15:  # In start of simulatiom all robots will directed to gradient
                weight_forces = 0.5
                weight_target = 1
    ...
    if t > 2:
        dist_traveled = dist_btw_points(com_flock_pos[-1], com_flock_pos[1])
    t += 1
...
b = [1.0 / n] * n
a = 1
#mod_comp_fitness = lfilter(b, a, spat_fitness)
#mod_infla_fitness = lfilter(b, a, inflammation)
spat_fitness = [x for x in spat_fitness if (math.isnan(x) != True)]
inflammation = [x for x in inflammation if (math.isnan(x) != True)]
mod_comp_fitness = savgol_filter(spat_fitness, 11, 7)
mod_infla_fitness = savgol_filter(inflammation, 11, 7)
#mod_infla_fitness = savgol_filter(inflammation, 11, 7)
plot1 = plt.figure(1)
plt.title('Compactness of ' + str(num_robots) + ' robots')
plt.xlabel('Iteration #')
plt.ylabel('Compactness of Robots')
plt.plot(list(range(len(mod_comp_fitness))), mod_comp_fitness, color='b', label='Compactness')
...
plt.show()
plot2 = plt.figure(2)
plt.title('IR Inflammation of Flock ' + str(num_robots))
plt.xlabel('Iteration #')
plt.ylabel('Combine Ir value of' + str(num_robots) + 'robots')
plt.plot(list(range(len(mod_infla_fitness))), mod_infla_fitness, color='r', label='Inflammation')
plt.show()
plot3 = plt.figure(3)
plt.title('Combine inflammation (Compactness & IR) ' + str(num_robots))
plt.xlabel('Iteration #')
plt.ylabel('Gcc + Gsc')
plt.plot(list(range(len(f))), lfilter(b, a, f))
plt.show()
print('Number of robots remain in flock', max_cluster_size)
print('Distance traveled by flock is {}m'.format(flock_dist))
print('Time consumed to reach goal is {}sec '.format(vrep_t))
